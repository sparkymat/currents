...
RETURNING id, user_id, name, description, created_at, updated_at, keywords
`
...
SELECT t.id, t.user_id, t.name, t.description, t.created_at, t.updated_at, t.keywords
  FROM media_item_topics mit
  INNER JOIN media_items mi ON mi.id = mit.media_item_id
...
SELECT id, user_id, name, description, created_at, updated_at, keywords
  FROM topics
  WHERE user_id = $1::uuid
    AND name ILIKE '%' || $2::text || '%'
  ORDER BY name ASC
