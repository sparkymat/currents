...
func (q *Queries) AddTopicToMediaItem(ctx context.Context, arg AddTopicToMediaItemParams) error {
	_, err := q.db.Exec(ctx, addTopicToMediaItem, arg.MediaItemID, arg.TopicID)
	return err
}
...
const fetchMediaItemTopicsForMediaItem = `-- name: FetchMediaItemTopicsForMediaItem :many
...
func (q *Queries) FetchMediaItemTopicsForMediaItem(ctx context.Context, arg FetchMediaItemTopicsForMediaItemParams) ([]MediaItemTopic, error) {
	rows, err := q.db.Query(ctx, fetchMediaItemTopicsForMediaItem, arg.MediaItemID, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const fetchTopicsByID = `-- name: FetchTopicsByID :many
...
func (q *Queries) FetchTopicsByID(ctx context.Context, arg FetchTopicsByIDParams) ([]Topic, error) {
	rows, err := q.db.Query(ctx, fetchTopicsByID, arg.TopicIds, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const scanMediaItemForTopics = `-- name: ScanMediaItemForTopics :many
SELECT DISTINCT t.id, t.user_id, t.name, t.description, t.created_at, t.updated_at FROM topics t
...
func (q *Queries) ScanMediaItemForTopics(ctx context.Context, mediaItemID uuid.UUID) ([]Topic, error) {
	rows, err := q.db.Query(ctx, scanMediaItemForTopics, mediaItemID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateTopicKeywords = `-- name: UpdateTopicKeywords :one
UPDATE topics
  SET keywords = $1::text[]
WHERE user_id = $2::uuid AND id = $3::uuid
RETURNING id, user_id, name, description, created_at, updated_at
`
...
func (q *Queries) UpdateTopicKeywords(ctx context.Context, arg UpdateTopicKeywordsParams) (Topic, error) {
	row := q.db.QueryRow(ctx, updateTopicKeywords, arg.Keywords, arg.UserID, arg.TopicID)
	var i Topic
	err := row.Scan(
		&i.ID,
		&i.UserID,
